from new_movies import movies_ranking
from new_movies.random_data_utility import random_generator


def my_formatter(sentence, /, **kwargs):
    for key, value in kwargs.items():
        sentence = sentence.replace(f"<{key}>", value)
    return sentence


def run_example():
    # some_movies = random_generator.generate_random_movies(movies_number=15)
    # movies_ranking.print_top_movies(some_movies)
    # movies_ranking.print_top_movies(some_movies, move_limit=15)
    print(my_formatter("<greeting>, my name is <name>", greeting="Hello", name="Mikołaj"))


if __name__ == "__main__":
    run_example()
